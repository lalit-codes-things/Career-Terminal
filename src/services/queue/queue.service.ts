...
import {
  ...
  type InterviewPatternMiningJobPayload,
} from './queue.types';
...
export class QueueService {
  ...
  private readonly interviewPatternMiningQueue: Queue<InterviewPatternMiningJobPayload>;
  ...
  constructor() {
    ...
    this.interviewPatternMiningQueue = new Queue<InterviewPatternMiningJobPayload>(
      QUEUE_NAMES.INTERVIEW_PATTERN_MINING,
      conn,
    );
  ...
  async addInterviewPatternMiningJob(
    payload: InterviewPatternMiningJobPayload,
    opts: JobsOptions = {},
  ): Promise<string> {
    const job = await this.interviewPatternMiningQueue.add(payload.type, payload, {
      ...DEFAULT_JOB_OPTIONS,
      ...opts,
    });
    logger.info('[QueueService] Interview pattern mining job enqueued', {
      jobId: job.id,
      type: payload.type,
    });
    return job.id!;
  ...
  async scheduleInterviewPatternMiningRepeatableJobs(): Promise<void> {
    const commonOpts = { pattern: '0 0 * * 0' };

    await this.interviewPatternMiningQueue.upsertJobScheduler(
      'MINE_FREQUENT_COMPETENCIES',
      commonOpts,
      {
        name: 'MINE_FREQUENT_COMPETENCIES',
        data: { type: 'MINE_FREQUENT_COMPETENCIES' },
      },
    );
    ...
    await this.interviewPatternMiningQueue.upsertJobScheduler(
      ...
      commonOpts,
      {
        name: 'MINE_FAILURE_MODES',
        data: { type: 'MINE_FAILURE_MODES' },
      },
    );

    logger.info('[QueueService] Interview pattern mining repeatable jobs scheduled');
  }
  ...
  async getDepths(): Promise<Record<string, number>> {
    const [
      ...
      this.interviewPatternMiningQueue.getJobCounts(),
    ]);
    return {
      ...
      interviewPatternMining:
        (interviewPatternMining.waiting ?? 0) + (interviewPatternMining.active ?? 0) + (interviewPatternMining.delayed ?? 0),
    };
  }
  ...
  async close(): Promise<void> {
    await Promise.all([
      ...
      this.interviewPatternMiningQueue.close(),
    ]);
