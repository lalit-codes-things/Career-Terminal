/**
 * Country / ISO 3166-1 normalization and validation.
 *
 * Provides an ISO 3166-1 alpha-2 / alpha-3 catalogue plus common-name aliases
 * ("UK" -> "GB", "USA" -> "US") used to normalize provider country fields.
 */
...
/** Full ISO 3166-1 country catalogue (alpha-2/alpha-3/English name). */
export const ISO_COUNTRIES: readonly CountryEntry[] = [
  ...
  { alpha2: 'CD', alpha3: 'COD', name: 'Congo (Democratic Republic)' },
  ...
  { alpha2: 'KP', alpha3: 'PRK', name: 'Korea (North)' },
  { alpha2: 'KR', alpha3: 'KOR', name: 'Korea (South)' },
...
const ALPHA2_TO_ENTRY: ReadonlyMap<string, CountryEntry> = new Map(
  ISO_COUNTRIES.map((c) => [c.alpha2, c]),
);
const ALPHA3_TO_ENTRY: ReadonlyMap<string, CountryEntry> = new Map(
  ISO_COUNTRIES.map((c) => [c.alpha3, c]),
);

/** Common non-ISO aliases providers use for country codes. */
...
/** True when the string is a valid ISO 3166-1 alpha-2 code. */
export function isCountryAlpha2(code: string): boolean {
  return ALPHA2_TO_ENTRY.has(code.toUpperCase());
}

/** True when the string is a valid ISO 3166-1 alpha-3 code. */
export function isCountryAlpha3(code: string): boolean {
  return ALPHA3_TO_ENTRY.has(code.toUpperCase());
}

/** True when the string is a valid ISO 3166-1 alpha-2 or alpha-3 code. */
export function isValidCountryCode(code: string): boolean {
  return isCountryAlpha2(code) || isCountryAlpha3(code);
}

/**
 * Normalize a country field to an ISO 3166-1 alpha-2 code.
 * Accepts alpha-2, alpha-3 and common name aliases.
...
export function normalizeCountryCode(value: string): string | null {
  ...
  if (ALPHA2_TO_ENTRY.has(cleaned)) {
    return cleaned;
  }
  if (ALPHA3_TO_ENTRY.has(cleaned)) {
    return ALPHA3_TO_ENTRY.get(cleaned)!.alpha2;
  }
  ...
  // Name lookup (last word of the cleaned string matching a country name).
  const byName = ISO_COUNTRIES.find((c) => c.name.toUpperCase() === cleaned);
  return byName?.alpha2 ?? null;
}

/** Look up the ISO alpha-3 code for an alpha-2 code (or null). */
export function countryCodeToAlpha3(code: string): string | null {
  const entry = ALPHA2_TO_ENTRY.get(code.toUpperCase());
