...
export class CrossEpicIntelligenceIntegrationService {
  private readonly config: CrossEpicIntegrationConfig;
  ...
  constructor(config?: Partial<CrossEpicIntegrationConfig>) {
    this.config = { ...DEFAULT_CONFIG, ...config };
  }

  getConfig(): Readonly<CrossEpicIntegrationConfig> {
    return this.config;
  }
  ...
  getStats(): CrossEpicIntelligenceStats {
    const links = [...this.links.values()];
    const activeLinks = links.filter((l) => !this.isExpired(l));

    const linksByDomain: Record<IntelligenceDomain, number> = {} as Record<IntelligenceDomain, number>;
    const linksBySourceEpic: Record<EpicId, number> = {} as Record<EpicId, number>;
    const linksByTargetEpic: Record<EpicId, number> = {} as Record<EpicId, number>;
  ...
  async getBundle(sourceEpic: EpicId, sourceEntityId: string): Promise<CrossEpicIntelligenceBundle> {
  ...
  private isExpired(link: CrossEpicIntelligenceLink): boolean {
    const ttlMs = link.evidence[0]?.confidence ? this.config.defaultTtlMs : this.config.defaultTtlMs;
    return Date.now() - link.createdAt.getTime() > ttlMs;
  }
}