...
export function buildDefaultTemplates(): PromptTemplate[] {
  ...
  return [
    ...
    // ─── Job Email Classification ───────────────────────────────────────────────
    {
      ...
      name: 'Job Email Classification',
      version: '1.0.0',
      tier: 'fast',
      systemPrompt: [
        'You are an expert career-intelligence classifier.',
        'Classify the email into one of the following categories:',
        'Job Application, Interview Invitation, Rejection, Offer, Recruiter Outreach,',
        'Assessment/Test, Networking, Career Newsletter, or Not Job Related.',
        'Also extract the company name and job role title if present.',
        'Return ONLY valid JSON. Do not invent entities.',
        'confidence is a float from 0.0 (no confidence) to 1.0 (certain).',
      ].join('\n'),
      userPromptTemplate: [
        ...
        '    {',
        '      "field": "category",',
        '      "value": "Job Application|Interview Invitation|Rejection|Offer|Recruiter Outreach|Assessment/Test|Networking|Career Newsletter|Not Job Related",',
        '      "rawValue": "<exact text that supports the category>",',
        '      "confidence": <0.0-1.0>,',
        '      "evidence": [{"excerpt": "<exact quote>", "confidence": <0.0-1.0>}]',
        '    },',
        '    {',
    ...
    // ─── Resume Extraction ────────────────────────────────────────────────────────
    {
      ...
      systemPrompt: [
        ...
        'Extract each skill, technology, occupation, experience entry, and education entry as individual field items.',
        'Only extract entities explicitly mentioned in the text. Do not infer or hallucinate.',
        'For experience entries, include role, company, dates, and years if available.',
        'For education entries, include degree, field of study, institution, and year if available.',
        'confidence is a float from 0.0 (no confidence) to 1.0 (certain).',
        'Return ONLY valid JSON.',
      ].join('\n'),
      userPromptTemplate: [
        ...
        '      "field": "skill",',
        '      "value": "<skill name>",',
        '      "rawValue": "<exact text from resume>",',
        '      "confidence": <0.0-1.0>,',
        '      "evidence": [{"excerpt": "<exact quote from resume>", "confidence": <0.0-1.0>}]',
        '    },',
        '    {',
        '      "field": "technology",',
        ...
        '    {',
        '      "field": "experience",',
        '      "value": {"role": "<role or null>", "company": "<company or null>", "dates": "<dates or null>", "years": <number>},',
        '      "rawValue": "<exact text for this experience entry>",',
        '      "confidence": <0.0-1.0>,',
        '      "evidence": [{"excerpt": "<exact quote>", "confidence": <0.0-1.0>}]',
        '    },',
        '    {',
        '      "field": "education",',
        '      "value": {"degree": "<degree or null>", "field": "<field of study or null>", "institution": "<institution or null>", "year": "<year or null>"},',
        '      "rawValue": "<exact text for this education entry>",',
        '      "confidence": <0.0-1.0>,',
        '      "evidence": [{"excerpt": "<exact quote>", "confidence": <0.0-1.0>}]',
        '    }',
        '  ]',
    ...
    // ─── Economic Document Extraction ──────────────────────────────────
    {
      templateId: 'economic-document-extraction',
      name: 'Economic Document Extraction',
      version: '1.0.0',
      tier: 'powerful',
      systemPrompt: [
        'You are an expert economic document extraction system for Career Terminal.',
        'Extract structured economic data from any economic document: offer letters,',
        'pay stubs, equity grants, promotion letters, tax forms, contractor agreements,',
        'severance agreements, HR letters, etc.',
        'Treat offers as just one document type among many. Extract all economic',
        'components generically without assuming a specific document type.',
        'Return ONLY valid JSON. Do not hallucinate facts.',
        'Every fact must have evidence pointing to the exact excerpt that supports it.',
        'confidence is a float from 0.0 (no confidence) to 1.0 (certain).',
      ].join('\n'),
      userPromptTemplate: [
        'Extract economic data from the following document.',
        '',
        ...
        'VALID TO: {{validTo}}',
        'TRANSACTION START: {{transactionStart}}',
        'TRANSACTION END: {{transactionEnd}}',
        '',
        'DOCUMENT CONTENT:',
        '{{content}}',
        '',
        'Extract ALL economic components found in this document.',
        'Return JSON with this exact structure:',
        '{',
        '  "fields": [',
        '    {',
        '      "field": "<field_name>",',
        '      "value": <extracted_value>,',
        '      "rawValue": "<exact text from document>",',
        '      "confidence": <0.0-1.0>,',
        '      "evidence": [{"excerpt": "<exact quote>", "confidence": <0.0-1.0>}]',
        '    }',
        '  ]',
        '}',
        '',
        'Valid field names include but are not limited to:',
        'base_salary, hourly_rate, annual_compensation, total_compensation,',
        'signing_bonus, retention_bonus, performance_bonus, stock_grant,',
        'rsu, equity_value, equity_vesting_schedule, allowance, stipend,',
        'commission_structure, commission_rate, commission_target,',
        'benefits_value, health_insurance, retirement_match, pto_days,',
        'vacation_days, sick_leave, parental_leave, relocation_allowance,',
        'housing_allowance, transportation_allowance, education_allowance,',
        'contract_value, contract_duration, contract_type,',
        'effective_date, start_date, end_date, notice_period,',
        'job_title, job_level, department, reporting_manager,',
        'company_name, company_location, work_location,',
        'employment_type, work_arrangement, remote_policy,',
        'negotiation_terms, offer_expiration, acceptance_deadline,',
        'hiring_budget_strength, recruiter_urgency,',
        'market_comparison, internal_equity,',
        'promotion_probability, salary_compression, offer_competitiveness,',
        'negotiation_leverage, career_wealth_trajectory.',
      ].join('\n'),
      outputSchema: {
        type: 'object',
        required: ['fields'],
    ...
    },
    {
      ...
      name: 'Interview Transcript Extraction',
      version: '1.0.0',
      tier: 'powerful',
      systemPrompt: [
        'You are an expert interview intelligence extraction system.',
        'Extract structured interview data from transcripts or notes.',
        'Identify rounds, questions, responses, demonstrated competencies, signals, and outcomes.',
        'Return ONLY valid JSON. Do not hallucinate facts.',
        'Every extracted item must have evidence pointing to the exact excerpt that supports it.',
        'confidence is a float from 0.0 (no confidence) to 1.0 (certain).',
      ].join('\n'),
      userPromptTemplate: [
        'Extract structured interview intelligence from the following transcript or notes.',
        '',
        ...
        '      "field": "<field_name>",',
        '      "value": <extracted_value>,',
        '      "rawValue": "<exact text from transcript>",',
        '      "confidence": <0.0-1.0>,',
        '      "evidence": [{"excerpt": "<exact quote>", "confidence": <0.0-1.0>}]',
        '    }',
        '  ]',
        '}',
        '',
        'Valid field names and value shapes:',
        '- session: object with roleTitle, jobLevel, loopType, sourceType, status, finalDecision, offerExtended, offerAccepted',
        '- round: object with roundType, sequenceNumber, interviewerLabel, durationMinutes, outcomeScore, outcomeLabel, notes',
        '- event: object with eventType (QUESTION_ASKED|RESPONSE_GIVEN|DECISION_MADE|OFFER_EXTENDED|OFFER_DECLINED|FEEDBACK_RECEIVED), eventCategory, title, description, effectiveDate, sequenceNumber',
        '- competency: object with competency (key), name, category, demonstratedLevel (0-1), evidenceExcerpt',
        '- signal: object with signalType (COMPETENCY_SIGNAL|STRESS_SIGNAL|COMMUNICATION_PATTERN|BIAS_RISK|NEGOTIATION_LEVERAGE), signalCategory, signalName, signalValue (object), sourceEventId',
        '- decision: object with finalDecision (HIRE|NO_HIRE|EXTEND|DECLINE|HOLD), confidence',
        '',
        'Round types: SCREENING, TECHNICAL, BEHAVIORAL, CASE_STUDY, SYSTEM_DESIGN, PRESENTATION, PANEL, FINAL',
        'Signal categories: question, response, decision, negotiation, followup',
      ].join('\n'),
      outputSchema: {
        type: 'object',
        required: ['fields'],
