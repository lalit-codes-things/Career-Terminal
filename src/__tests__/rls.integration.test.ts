...
describeRlsIntegration('PostgreSQL RLS Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    adminPool = new Pool({
      connectionString: getConnUrl(
        'career_terminal_migr',
        INTEGRATION_URL!.split(':')[2]?.split('@')[0] || 'password',
      ),
    });
    ...
    await adminPool.query(
      `INSERT INTO "users" (id, email, name, region) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING`,
    ...
    await adminPool.query(
      `INSERT INTO "candidate_profiles" (id, user_id, full_name) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING`,
    ...
    // Create user resumes
    await adminPool.query(
      `INSERT INTO "user_resumes" (id, user_id, filename, s3_key, content_type) VALUES ($1, $2, $3, $4, $5)`,
    ...
    // Create events
    await adminPool.query(
      `INSERT INTO "events" (id, event_type, aggregate_id, aggregate_type, user_id, cell_id, payload, correlation_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
  ...
  afterAll(async () => {
    ...
    if (adminPool) {
      await adminPool.query('DELETE FROM events WHERE user_id = ANY($1::uuid[])', [
        [userAId, userBId],
      ]);
      await adminPool.query('DELETE FROM user_resumes WHERE user_id = ANY($1::uuid[])', [
        [userAId, userBId],
      ]);
      await adminPool.query('DELETE FROM candidate_profiles WHERE user_id = ANY($1::uuid[])', [
        [userAId, userBId],
      ]);
  ...
  describe('Cross-user isolation', () => {
    ...
    it("User A CANNOT read User B's candidate profile", async () => {
      await runtimePoolA.query('SELECT set_app_user_id($1)', [userAId]);
      const result = await runtimePoolA.query(
        'SELECT COUNT(*) FROM "candidate_profiles" WHERE "user_id" = $1',
        [userBId],
      );
    ...
    it("User A CANNOT update User B's resume", async () => {
      await runtimePoolA.query('SELECT set_app_user_id($1)', [userAId]);
      const bResume = await adminPool.query(
        'SELECT id FROM user_resumes WHERE user_id = $1 LIMIT 1',
        [userBId],
      );
      ...
      const rowsAfter = await adminPool.query('SELECT id FROM user_resumes WHERE user_id = $1', [
        userBId,
      ]);
      ...
      try {
        await runtimePoolA.query('UPDATE "user_resumes" SET filename = $1 WHERE id = $2', [
          'hacked.pdf',
          targetId,
        ]);
        // Should have thrown or affected 0 rows
        const check = await adminPool.query('SELECT filename FROM user_resumes WHERE id = $1', [
          targetId,
        ]);
    ...
    it("User A CANNOT delete User B's event", async () => {
      await runtimePoolA.query('SELECT set_app_user_id($1)', [userAId]);
      const bEvents = await adminPool.query('SELECT id FROM events WHERE user_id = $1 LIMIT 1', [
        userBId,
      ]);
      ...
      const targetEvent = await adminPool.query(
        'SELECT id FROM events WHERE user_id = $1 LIMIT 1',
        [userBId],
      );
      ...
      try {
        ...
        const check = await adminPool.query('SELECT COUNT(*) FROM events WHERE id = $1', [
          targetId,
        ]);
